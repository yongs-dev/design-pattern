...
/**
 * <b>싱글톤 패턴 (Singleton Pattern)</b><br>
 * 단 하나의 유일한 객체를 만들기 위한 코드 패턴으로 메모리 절약을 위해 인스턴스가 필요할 때 똑같은 인스턴스를 새로 만들지 않고 기존의 인스턴스를 가져와 활용하는 기법<br>
 * 리소스를 많이 차지하는 역할을 하는 무거운 클래스일 때 적용하기 적합하다.<br>
 * 대표적으로 데이터베이스 연결 모듈을 들 수 있는데 I/O 바운드 그 자체로 무거운 작업에 속하며 한 번만 객체를 생성하고 돌려쓰면 되기 때문이다.<br>
 * 이 밖에도 디스크 연결, 네트워크 통신, DBCP 커넥션 풀, 스레드 풀, 캐시, 로그 기록 객체 등에 이용된다.<br>
 * <br><hr><br>
 * <b>싱글톤 패턴 구현 기법</b><br>
